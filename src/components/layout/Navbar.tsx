    ...
    <header className={`sticky top-0 z-40 w-full transition-all ${isScrolled ? "bg-white shadow-md py-2" : "bg-white/80 backdrop-blur-md py-4"}`}>
      <div className="container flex items-center justify-between px-4 md:px-6">
        ...
        <div className="flex items-center gap-4">
          <a href="tel:+74957990926" className="hidden md:flex items-center text-sm font-medium gap-2 hover:text-brand">
            <Phone className="h-4 w-4" />
            +7 (495) 799-09-26
          </a>

          <Button variant="ghost" size="icon" className="hidden md:flex" onClick={() => setIsChatOpen(true)}>
          ...
          <Sheet open={isMobileMenuOpen} onOpenChange={setIsMobileMenuOpen}>
            ...
            <SheetContent side="right" className="w-full sm:w-80">
              <div className="flex flex-col space-y-4 pt-6">
                ...
                {navigation.map(item => <Link key={item.name} to={item.href} className={`py-2 ${isActive(item.href) ? "text-brand font-medium" : "text-muted-foreground"}`} onClick={() => setIsMobileMenuOpen(false)}>
                    {item.name}
                  </Link>)}
                <div className="pt-4">
                ...
                <a href="tel:+74957990926" className="flex items-center gap-2 py-2 text-muted-foreground">
                  <Phone className="h-4 w-4" />
                  +7 (495) 799-09-26
                </a>
